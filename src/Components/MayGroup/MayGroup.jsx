...
const MayGroup = () => {
    ...
    return (
        <div className="all">
            ...
            {
                groupList.length == 0 ?
                    <h2 className=" text-center font-bold capitalize text-red-400">No Group</h2>
                    :
                    showRequest ?
                        <div className=" bg-blue-500 p-8 rounded-lg relative">
                            <button onClick={() => setShowRequest(!showRequest)} className="Button_v_4 rounded-md absolute right-2 top-2">close</button>
                                ...
                                groupRequestJoinList.map((item,i)=>{
                                    return(
                                        ...
                                        <div className="group">
                                            ...
                                            <div className="text">
                                                <h2 className=" text-sm font-bold text-red-500 ">{item.userName}</h2>
                                            
                                            </div>
                                        </div>
                                        <div className="btn flex gap-4">
                                            <button onClick={()=>handleGroupAccept(item)} className="Button_v_5">accept</button>
                                            <button onClick={()=>handleRejectGroup(item)} className="Button_v_3">reject</button>
                        ...
                        showGroupInfo?
                        <div className=" bg-blue-500 p-8 rounded-lg relative">
                            <button onClick={() => setShowGroupInfo(!showGroupInfo)} className="Button_v_4 rounded-md absolute right-2 top-2">close</button>
                        ...
                        groupList.map((item, i) => {
                            return (
                                <div key={i} className="main_group">
                                    <div className="group">
                                        <div className="profile flex justify-center items-center">
                                            <h2 className="text-white font-bold text-lg uppercase"> {item.adminName[0]} </h2>
                                        </div>
                                        <div className="text">
                                            <h2 className=" text-sm font-bold text-red-500 ">{item.adminName}</h2>
                                            <h3 className="text-lg font-serif text-teal-600 ">{item.groupName}</h3>
                                            <h2>{item.groupIntroName}</h2>
                                        </div>
                                    </div>
                                    <div className="btn flex gap-3">
                                        <button onClick={()=>handleGroupInfo(item)} className="Button_v_5">info</button>
                                        <button onClick={() => handleGroupRequest(item)} className="Button_v_3">request</button>
                                        <button onClick={() => handleGroupDelet(item)} className="Button_v_4">delete</button>
                                    </div>
                                </div>
                            )
                        })
